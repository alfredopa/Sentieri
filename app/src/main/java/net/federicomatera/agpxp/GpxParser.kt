...
class GpxParser {
    ...
    fun parse(stream: InputStream): Gpx {
        ...
        if (firstChild == null || GPX_NODE != firstChild.nodeName) {
            while (doc.hasChildNodes() &&  GPX_NODE != firstChild.nodeName) {
            ...
            if (!nodeGpxOk)
                throw IOException("File GPX non non valido")
        }
        ...
        if (nodes != null) {
            for (idx in 0 until nodes.length) {
                ...
                when (currentNode.nodeName) {
                    ...
                    NUMBER_NODE -> number = currentNode.firstChild?.nodeValue?.toIntOrNull() ?: 0
